...
class YOLO(object):
    ...
    def __init__(self, **kwargs):
        ...
        #   调用摄像头
        self.capture = cv2.VideoCapture("./img/2.avi")
    # -------------------------------------#
    ...
    def detect_image(self):
        ...
        while(True):
            ...
            if len_box_boxes==0:
                ...
                current_boxes = np.insert(out_boxes, 4, values=0, axis=1)  # insert number label
                number_label = np.arange(len_box_boxes)
                current_boxes[:, 4] = number_label
                len_pre_box=len(pre_boxes)
                if len_pre_box==0:
                    current_boxes[:, 4] = number_label
                else:
                    for i in range(len_pre_box):
                        distance=current_boxes[0,1]-pre_boxes[i,1]
                        if abs(distance)<threshold:
                            current_boxes[:,4]+=pre_boxes[i,4]
                            print("pre: ,distance:  ",(i,distance))
                            break
            # ---------------------------------------------------------#
            ...
            for i, c in list(enumerate(current_boxes)):
                predicted_label = int(current_boxes[i,4])   #number label
